...
def runExperiment(params, code):
    ...
    output = "Error"
    try:
        output = subprocess.check_output("bash "+code, shell=True)
    except:
        dobehlo=False
...
def executeall(script, nakom=None): #todo
    """execute script on list of files (default-all)"""
    os.chdir(gitwork)
    zoznam = getfile('runs')
    if nakom is None:
